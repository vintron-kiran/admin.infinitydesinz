...
import { fetchBrands, addBrand, updateBrand,deleteBrand } from './mock-brands';
...
export default function BrandsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    //    setBrands(prev => [...prev, added]);
      ...
      const updated = await fetchBrands(); // ✅ Always fetch fresh list
    setBrands(updated);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleBulkStatusChange = async  (status: boolean) => {
    ...
    selectedIds.map((id) => {
      ...
      return updateBrand(id, { name, logo_url: logo_url || '', status });
    })
  );
  ...
  return (
    <div className="container-fluid px-5 py-4" style={{ maxWidth: '100%', margin: '0 auto' }}>
      ...
      {showForm ? (
        <div className="card mb-4 p-3">
          <div className="card-body">
            <h5>{editingId ? 'Edit Brand' : 'Create a Brand'}</h5>
          ...
          <div className="card-body">
            ...
            <div className="table-responsive">
              <table className="table table-striped table-hover align-middle">
                <thead>
                  <tr>
                    <th><input type="checkbox" onChange={handleSelectAll} checked={paginatedBrands.every(b => selectedIds.includes(b.id!))} /></th>
                    <th>S.No</th>
                    <th>Brands</th>
                    <th>Status</th>
                    <th>Action</th>
